...
async function runQuery_test(){
    try{
        ...
        const output = await Data.createUserWithOutbox('testUser', '1234512345', 'testEmail@test.test', 1);
        ...
        //verify user
        const userCheck = await Data.getQuery("SELECT * FROM users WHERE username = ?", ['testUser']);

        //verify outbox
        const outboxCheck = await Data.getQuery("SELECT * FROM Outbox WHERE username = ?", ['testUser']);

        //if user ad outbox were both inserted and retrieved then it passed
        if (userCheck.length > 0 && outboxCheck.length > 0) {
            console.log("ntegration Test Passed: User and Outbox record created.");
            process.exit(0);
        } else {
